...
- `src/stocks.js` — gestiunea stocurilor cantitativ-valoric la CMP/FIFO: fișă de magazie, stoc
  curent, transferuri fără impact financiar și detectarea cantității cerute peste disponibil.
- `src/stocksService.js` — **service layer** pentru scrierile de stocuri (nomenclatoare, mișcări,
  postare/storno append-only, stoc inițial, inventariere/storno): rutele din `src/routes/stocks.js` sunt doar
...
- `src/payroll.js` — salarizare: nomenclator angajați (cu **spor** impozabil, **avans** → 425 și
  **rețineri** din net → 427) + stat de plată per angajat (CAS/CASS/impozit/CAM, rest de plată),
  ...
  repostează cronologic, iar plata integrală este unică și cere un stat postat. D112 XML/PDF,
  validarea pre-depunere, dosarul FNUASS și documentele salariale finale sunt blocate până există
  fotografia activă completă; previzualizarea explicită `live=1` este marcată vizibil „CIORNĂ” și
  nu poartă rubrică de semnătură. Din aceste revizii
...
- **E2E pe live:** `npm run e2e` (`scripts/e2e.mjs`, Playwright — pe acest server prin Docker,
  comanda e în antetul scriptului): 74 verificări cap-coadă pe instanța reală, cu contul demo —
...
- **E2E pe instanță izolată:** `npm run e2e-izolat` (`scripts/e2e-izolat.sh` +
  ...
  rulează **171 verificări pe instanță izolată** (unele verifică fiecare pagină/declarație,
