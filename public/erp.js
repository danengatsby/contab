...
(function () {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  //  COMUTATORUL DE ASPECT
  //
  //  Butonul se injecteaza INTOTDEAUNA, si cand chrome-ul e oprit: altfel aspectul
  //  modern ar fi o fundatura — ai putea iesi din cel clasic, dar nu te-ai mai
  //  putea intoarce decat stergand datele din browser. Un comutator care merge
  //  intr-o singura directie nu e un comutator.
  // ─────────────────────────────────────────────────────────────────────────────
  var CHEIE = 'contab.aspect'; // 'clasic' (implicit) | 'modern'

  function aspect() {
    try { return localStorage.getItem(CHEIE) === 'modern' ? 'modern' : 'clasic'; }
    catch (e) { return 'clasic'; } // localStorage blocat (mod privat, politici) — nu e motiv sa cada nimic
  }

  function injecteazaComutator() {
    var gazda = $('#sideTools');
    if (!gazda || document.getElementById('erpSkinBtn')) return;
    var clasic = aspect() === 'clasic';
    var b = el('button', 'btn small');
    b.id = 'erpSkinBtn';
    b.type = 'button';
    b.textContent = clasic ? '🌐 Aspect modern' : '🖥️ Aspect clasic';
    b.title = clasic
      ? 'Treci la aspectul modern, cu spațiere largă'
      : 'Treci la aspectul clasic de aplicație de contabilitate, cu densitate mare';
    b.addEventListener('click', function () {
      try { localStorage.setItem(CHEIE, clasic ? 'modern' : 'clasic'); } catch (e) { /* vezi mai sus */ }
      location.reload();
    });
    gazda.appendChild(b);
  }
  ...
  function poate() { return window.innerWidth >= LATIME_MINIMA && aspect() === 'clasic'; }
