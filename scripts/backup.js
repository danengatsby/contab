...
async function main() {
  ...
  if (EMAIL_TO) {
    ...
    if (!RESEND_KEY) {
      ...
      try { await emailArchive(f.path, sizeLabel); offsiteOk = true; log('Offsite email OK ->', EMAIL_TO); }
  ...
  if (RCLONE_REMOTE) {
    ...
    const rc = spawnSync('rclone', ['copy', f.path, RCLONE_REMOTE], { encoding: 'utf8', timeout: 10 * 60 * 1000 });
