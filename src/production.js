...
// genereaza miscarile de stoc (consum la metoda firmei + receptie produs finit la cost) si
// articolul contabil: consum 6xx = 3xx (CMP/FIFO) + obtinere produse finite 345 = 711.
// Functie pura — nu salveaza; rezultatul e aplicat de ruta.

const { round2, roundQty } = require('./util');
const { cogsAccount, movementRow } = require('./stocks');
...
function buildProduction(products, baseMovements, order, opts) {
  ...
  // 1) consum materiale (iesire din stoc, la metoda firmei)
  for (const mat of (order.materiale || [])) {
    ...
    newMovements.push({
      ...
      cantitate: roundQty(Number(mat.cantitate) || 0), pretUnitar: 0, document: o.document || '', entryId: o.entryId || null, auto: true,
    });
  }
  const consumMovs = newMovements.slice();
  const all = (baseMovements || []).concat(newMovements);
  const byConsum = {}; let costMateriale = 0; const lipsuri = [];
  for (const mv of consumMovs) {
    const p = find(mv.productId);
    const row = movementRow(p, all, mv.id, o.metoda);
    const val = round2(row ? row.iesireV : 0);
    const efectiv = roundQty(row ? row.iesireQ : 0);
    const lipsa = roundQty(mv.cantitate - efectiv);
    mv.valoareContabila = val;
    if (val > 0) {
    ...
    if (lipsa > 0) {
      lipsuri.push({ productId: mv.productId, denumire: p.denumire || p.cod || mv.productId,
        cerut: mv.cantitate, descarcat: efectiv, lipsa });
      warns.push('Stoc insuficient pentru materialul „' + (p.denumire || p.cod || mv.productId)
        + '”: lipsesc ' + lipsa + '.');
    }
  }
  for (const k of Object.keys(byConsum)) {
    ...
    lines.push({ debit, credit, suma: byConsum[k], explicatie: 'Consum materiale în producție (' + met + ')' });
  }
  ...
  const qty = roundQty(Number(order.cantitate) || 0);
  ...
  newMovements.push({
    ...
    cantitate: qty, pretUnitar: costUnitar, valoareContabila: valoare,
  ...
  return { newMovements, lines, costMateriale, valoareObtinuta: valoare, warns, lipsuri };
}
...
function expandRecipe(recipe, cantitate, costUnitar) {
  ...
  return {
    ...
    materiale: (recipe.materiale || []).map((m) => ({
      productId: m.productId, gestiuneId: m.gestiuneId || null, cantitate: roundQty((Number(m.cantitate) || 0) * factor),
    })),
  };
}
...
function productionReport(db, period) {
  ...
  const rows = (db.stockMovements || [])
    ...
    .map((m) => {
      ...
      return { data: m.data, cod: p.cod || '', denumire: p.denumire || '', um: p.um || '', cantitate: roundQty(m.cantitate), cost: round2(m.pretUnitar), valoare: round2(m.cantitate * m.pretUnitar), document: m.document || '' };
    })
    .sort((a, b) => (a.data < b.data ? -1 : a.data > b.data ? 1 : 0));
  return { period, rows, totalValoare: round2(rows.reduce((s, r) => s + r.valoare, 0)), totalCantitate: roundQty(rows.reduce((s, r) => s + r.cantitate, 0)) };
}
