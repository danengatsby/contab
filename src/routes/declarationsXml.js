...
module.exports = function register(app, ctx) {
  ...
  function requireVatPayer(v, res, period) {
    if (fiscalProfile.profileAt(v, period).tvaPlatitor) return true;
  ...
  app.get('/xml/efactura/:id', (req, res) => {
    ...
    sendXml(res, xml.eFacturaXml(company, e, d.partners[fid] || {}), 'efactura-' + (e.document || e.id) + '.xml');
  });
  ...
  function recordDecl(req, tip, period, artifact, filename, balanceControlSnapshot) {
    ...
    const view = S(req);
    const profileSnapshot = fiscalProfile.declarationSnapshot(view, period, { angajati: view.angajati });
    const ruleSet = fiscal.rulesAt(period);
    const fiscalRuleReference = { ruleSetId: ruleSet.id, fiscalRulesHash: ruleSet.hash };
    const balanceCategorySnapshot = tip === 'bilant'
      ? (() => {
        const gate = balanceCategory.validateConfirmation(view, String(period).slice(0, 4));
        return {
          year: String(period).slice(0, 4), category: gate.category,
          confirmationId: gate.confirmation.id, inputHash: gate.confirmation.inputHash,
          confirmedAt: gate.confirmation.confirmedAt, confirmedBy: gate.confirmation.confirmedBy,
          calculatedCategory: gate.confirmation.calculatedCategory,
        };
      })() : null;
    const d = db.get(); const fid = activeId(req);
    const index = (d.declarations || []).findIndex((row) => String(row.firmaId) === String(fid)
      && row.tip === tip && row.period === period);
    const before = index >= 0 ? JSON.parse(JSON.stringify(d.declarations[index])) : null;
    try {
      const rec = decl.record(d, fid, tip, period, {
        status: 'generata', generatedAt, updatedBy: req.user && req.user.username,
        authorization: {
          authorized: true, action: 'declaration.prepare',
        ...
        profileSnapshot,
        ...fiscalRuleReference,
        ...(balanceCategorySnapshot ? { balanceCategorySnapshot } : {}),
        ...(tip === 'bilant' && balanceControlSnapshot ? { balanceControlSnapshot } : {}),
        artifact: {
      ...
      // Middleware-ul transversal scrie deja un singur `export.xml` după răspuns. Îi dăm ancora
      // dosarului în loc să dublăm fiecare descărcare în auditul viu.
      req.filingAuditDetail = ' · SHA-256 ' + rec.artifactHash.slice(0, 12)
        + ' · dosar ' + rec.dossier.id + ' · lanț ' + rec.stateChainHash;
      db.save();
    } catch (e) {
      if (before) d.declarations[index] = before;
      else d.declarations = (d.declarations || []).filter((row) => !(String(row.firmaId) === String(fid)
        && row.tip === tip && row.period === period));
      throw e;
    }
  }

  function sendRecordedXml(req, res, tip, period, out, filename, balanceControlSnapshot) {
    try { fiscalReview.assertReady('generarea artefactului de depunere ' + String(tip).toUpperCase() + ' ' + String(period || '')); }
    catch (e) { res.status(e.status || 409).send(e.message); return false; }
    recordDecl(req, tip, period, out, filename, balanceControlSnapshot);
  ...
  app.get('/xml/d300', (req, res) => {
    const v = S(req);
    if (!requireVatPayer(v, res, req.query.period)) return;
    const hv = fiscalProfile.viewAt(v, req.query.period || null);
    const pd300 = acc.vatPeriod(hv, req.query.period || null); // agregă după profilul perioadei
    ...
    const out = xml.d300Xml(hv.company, pd300, rep.d300(hv, pd300), declarantOf(req),
  ...
  app.get('/xml/d394', (req, res) => {
    ...
    const pd394 = acc.vatPeriod(hv, period); // agregă după profilul perioadei
    const out = xml.d394Xml(hv.company, pd394, acc.vatJournals(hv, pd394), declarantOf(req), rep.achizitiiPfCarnet(hv, pd394));
  ...
  app.get('/pdf/d394', (req, res) => {
    ...
    if (!requireVatPayer(v, res, req.query.period)) return;
    const hv = fiscalProfile.viewAt(v, req.query.period || null);
    const pd394 = acc.vatPeriod(hv, req.query.period || null);
    return pdf.d394Pdf(res, hv.company, {
      period: pd394,
      ops: xml.d394Operatiuni(acc.vatJournals(hv, pd394), rep.achizitiiPfCarnet(hv, pd394)),
    });
  });
  app.get('/pdf/saft', (req, res) => {
    const year = req.query.year || String(new Date().getFullYear());
    const hv = fiscalProfile.viewAt(S(req), year);
    return pdf.saftPdf(res, hv.company, saft.saftSummary(hv, year));
  });
  ...
  app.get('/xml/d301', (req, res) => {
    ...
    try { out = xml.d301Xml(hv.company, period, d301.report(hv, period), declarantOf(req), { rectificativa: depusaDeja }); }
  ...
  app.get('/xml/d307', (req, res) => {
    ...
    let out;
    try {
      out = xml.d307Xml(hv.company, period, d307.report(hv, period), declarantOf(req), {
  ...
  app.get('/xml/d311', (req, res) => {
    ...
    let out;
    try {
      out = xml.d311Xml(hv.company, period, d311.report(hv, period), declarantOf(req), {
        rectificativa: depusaDeja,
        dupaRezerva: req.query.dupaRezerva === '1',
        temei: Number(req.query.temei) === 2 ? 2 : 1,
      });
  ...
  app.get('/xml/d390', (req, res) => {
    const v = S(req);
    const hv = fiscalProfile.viewAt(v, req.query.period || null);
    const out = xml.d390Xml(hv.company, req.query.period || null, rep.d390(hv, req.query.period || null));
  ...
  app.get('/xml/d205', (req, res) => {
    ...
    const hv = fiscalProfile.viewAt(v, y);
    const rec = decl.find(db.get(), activeId(req), 'd205', y + '-12');
    const out = xml.d205Xml(hv.company, y, rep.d205(hv, y), declarantOf(req), {
  ...
  app.get('/xml/intrastat', (req, res) => {
    ...
    const hv = fiscalProfile.viewAt(v, period);
    const out = xml.intrastatXml(hv.company, period, rep.intrastat(hv, period));
  ...
  app.get('/xml/d100', (req, res) => {
    ...
    const out = xml.d100Xml(hv.company, period, r, declarantOf(req));
  ...
  app.get('/xml/d710', (req, res) => {
    ...
    let out;
    try { out = xml.d710Xml(hv.company, period, initial, corrected, declarantOf(req)); }
    catch (e) { return res.status(400).send(e.message); }
  ...
  // Situatiile financiare ANUALE (S1120/S1121/S1122). Categoria este persistata explicit pe
  // firma, distinct de regimul fiscal micro/profit; ruta nu accepta suprascriere ad-hoc.
  ...
  app.get('/xml/d107', (req, res) => {
    ...
    let out;
    try {
      out = xml.d107Xml(hv.company, raportD107(hv, year), declarantOf(req), {
        rectificativa: !!decl.lastSubmission(rec),
      });
    } catch (e) { return res.status(400).send(e.message); }
  ...
  app.get('/xml/d177', (req, res) => {
    ...
    try { fiscalReview.assertReady('generarea artefactului de depunere D177 ' + year); }
    catch (e) { return res.status(e.status || 409).send(e.message); }
    recordDecl(req, 'd177', year + '-12');
    sendXml(res, xml.d177Xml(hv.company, d), 'd177-' + year + '.xml');
  });

  app.get('/xml/bilant', (req, res) => {
    const v = S(req);
    const year = String(req.query.year || (new Date().getFullYear() - 1));
    // Categoria nu vine nici din regimul fiscal, nici din query, nici direct din campul-oglinda al
    // firmei. XML-ul foloseste numai decizia ANUALA confirmata, daca hash-ul indicatorilor si al
    // istoricului este inca actual. O modificare ulterioara a datelor invalideaza confirmarea.
    let categoryGate;
    try { categoryGate = balanceCategory.validateConfirmation(v, year); }
    catch (e) { return res.status(e.status || 409).send(e.message); }
    const categorie = categoryGate.category;
    ...
    // F10 nu se mai echilibreaza prin modificarea automata a rezultatului reportat. Orice
    // diferenta peste toleranta, cont nemapat, metadata obligatorie lipsa, ajustare cu hash expirat
    // sau ruptura F10↔balanta↔F20 opreste artefactul INAINTE de XML si de registrul depunerilor.
    if (s.blocaje && s.blocaje.length) {
      const unmapped = s.raportMapare && s.raportMapare.conturiNemapate || [];
      logAudit('bilant.blocat', year + ': ' + s.blocaje.join(' | ') + (unmapped.length
        ? ' | nemapate: ' + unmapped.map((x) => x.account + '=' + x.value).join(', ') : ''), { req });
      return res.status(409).json({
        error: 'Generarea bilanțului este blocată până la remedierea reconcilierii.',
        code: 'BILANT_RECONCILIATION_FAILED', blockers: s.blocaje,
        unmappedAccounts: unmapped, reconciliation: s.reconciliere,
      });
    }
    const out = xml.bilantXml(s);
    const filename = s.antet.formular.cod.toLowerCase() + '-bilant-' + year + '.xml';
    const balanceControlSnapshot = {
      schemaVersion: 1, year, toleranceLei: bilant.TOLERANTA_ROTUNJIRE_LEI,
      sourceHashes: { curent: bilant.sourceHash(v, year), precedent: bilant.sourceHash(v, Number(year) - 1) },
      reconciliation: s.reconciliere,
      mappingHashes: (v.balanceSheetMappings || []).filter((x) => [year, String(Number(year) - 1)].includes(String(x.year))).map((x) => x.hash).sort(),
      adjustmentHashes: (v.balanceSheetAdjustments || []).filter((x) => [year, String(Number(year) - 1)].includes(String(x.year))).map((x) => x.hash).sort(),
    };
    sendRecordedXml(req, res, 'bilant', year + '-12', out, filename, balanceControlSnapshot);
  });
  ...
  app.get('/xml/d112', (req, res) => {
    ...
    const hv = fiscalProfile.viewAt(v, period);
    let sp;
    try { sp = statPlataPostata(hv, period); }
    catch (e) {
    ...
    const out = xml.d112Xml(hv.company, period, sp, declarantOf(req),
  ...
  app.get('/xml/saft', wrap(async (req, res) => {
    ...
    const period = monthReq ? acc.vatPeriod(v, monthReq) : null;
    const year = req.query.year || String(new Date().getFullYear());
    ...
    const hv = fiscalProfile.viewAt(v, monthReq || year);
    const out = await saft.saftXmlAsync(hv, period || year, tip);
